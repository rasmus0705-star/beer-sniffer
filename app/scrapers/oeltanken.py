...
SIDE_PAUSE = 0.2     # sekunder mellem produktside-kald (høflig mod serveren)
SIDE_TIMEOUT = 12
...
def scrape_oeltanken():
    ...
    while True:
        ...
        for product in products:
            ...
            _need_fetch = (not is_smagekasse) and (volume is None or abv is None)
            if _need_fetch and _cached:
                if volume is None and _cached.get("vol") is not None:
                    volume = _cached["vol"]
    ...
    _cache_save()
    print(f"  [oeltanken] sidehentning: {_cache_fetches} hentet, {_cache_hits} fra cache")
    return items
